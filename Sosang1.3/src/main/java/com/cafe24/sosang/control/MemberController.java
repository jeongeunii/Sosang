...
//		if (member_info != null) {
//			mav.setView(new RedirectView("../../sosang", true));
//		} else {// 로그인이 실패하면 login폼으로 돌아간다.
//			mav.setViewName("member/loginForm.jsp");
...
//	}
	...
	@RequestMapping(value = "member/memberChangeInfoForm.do", method = RequestMethod.GET)
	...
	public ModelAndView update_member(MemberDTO dto, ModelAndView mav) throws Exception {
		System.out.println("controller-업데이트멤버");
	...
	public ModelAndView find_Password(MemberDTO dto, HttpServletRequest req, ModelAndView mav) throws Exception {
		dto.setId((String) req.getParameter("id"));
		dto.setEmail((String) req.getParameter("email"));

		// ModelAndView map구조
		mav.addObject("findPassword", memberService.findUserPassword(dto));
